...
export default function AdminClient() {
  ...
  if (err) return <div className="mx-auto max-w-6xl px-2 sm:px-6 py-8"><p className="text-sm text-rose-600 bg-rose-50 border border-rose-200 rounded-xl px-3 py-2 dark:bg-rose-400/10 dark:border-rose-400/30 dark:text-rose-400">{err}</p></div>;
    ...
    <div className="mx-auto max-w-6xl px-2 sm:px-6 py-6 sm:py-8">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2 text-xs">
          <Link href="/" className="rounded-full border border-zinc-200 px-4 py-2 hover:bg-zinc-50 dark:border-white/15 dark:hover:bg-white/5">Нүүр</Link>
          <Link href="/browse" className="rounded-full border border-zinc-200 px-4 py-2 hover:bg-zinc-50 dark:border-white/15 dark:hover:bg-white/5">Сорилго</Link>
        </div>
      </div>
      ...
      <div className="mt-4 flex gap-1.5 overflow-x-auto pb-1 -mx-4 px-4 sm:mx-0 sm:px-0 scrollbar-none">
        {[
          ...
          <button key={id} onClick={() => { setTab(id as any); if (id === "reports") fetchReports(); if (id === "payments") fetchPayments(); }} className={`shrink-0 rounded-full px-4 py-2.5 sm:py-2 text-xs sm:text-sm font-medium border min-h-[40px] ${tab === id ? "bg-indigo-600 text-white dark:bg-indigo-500/15 dark:text-indigo-200 dark:ring-1 dark:ring-inset dark:ring-indigo-400/25" : "bg-white border-zinc-200 hover:bg-zinc-50 dark:bg-white/[0.04] dark:border-white/15 dark:hover:bg-white/5"}`}>{label}</button>
        ))}
      </div>
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white p-4 sm:p-5 dark:bg-white/[0.04] dark:border-white/10">
            ...
            <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
              {Object.entries(stats.questions.byMain).sort((a,b)=>collator.compare(a[0],b[0])).map(([k,v])=>(
                <div key={k} className="flex justify-between rounded-xl bg-zinc-50 px-3 py-2 text-sm dark:bg-white/5"><span className="truncate pr-2">{k}</span><b>{v}</b></div>
              ))}
            </div>
          </div>
          <div className="rounded-2xl border border-zinc-200 bg-white p-4 sm:p-5 dark:bg-white/[0.04] dark:border-white/10">
              ...
              <table className="w-full text-xs sm:text-sm">
                ...
                <tbody>
                  {stats.questions.sources.sort((a,b)=>collator.compare(a.file,b.file)).map(s=>(
                    <tr key={s.file} className="border-t border-zinc-200 dark:border-white/10"><td className="py-2 pr-2 break-all">{s.file}</td><td className="py-2 text-right">{s.count}</td></tr>
                  ))}
                </tbody>
              </table>
            </div>
      ...
      {tab === "questions" && stats && (
        ...
        {(stats.questions.errors?.length || 0) > 0 && (
          <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4 sm:p-5 dark:bg-rose-400/10 dark:border-rose-400/30">
            ...
            <p className="mt-1 text-xs text-rose-600 dark:text-rose-300">Хариулт хоосон/буруу эсвэл 4-өөс өөр сонголттой — JSON файл дээр нь засаарай. Файл тус бүрээр бүлэглэв.</p>
