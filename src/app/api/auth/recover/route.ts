...
export async function POST(req: Request) {
  try {
    const { phone: rawPhone, code, newPassword } = await req.json();
    if (!rawPhone || !code || !newPassword) return NextResponse.json({ error: "Утас, код, шинэ нууц үг шаардлагатай" }, { status: 400 });
